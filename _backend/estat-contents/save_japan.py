...
from document_processer.main import JapanDocumentProcessorClass  # noqa: E402


class JapanDocument:
    """
    クラス:
        JapanDocument: 全国データの処理と保存を行うクラス
    ...
    def __init__(self, path: str):
        ...
        self.document = {
            ...
            **JapanDocumentProcessorClass(self.categories).result,
        }
        self.dir = self._create_directory()

    def _create_directory(self) -> tuple:
        dir = f"src/data/documents/{self.settings['fieldId']}/{self.settings['menuId']}/japan/{self.documentId}"
...
if __name__ == "__main__":
    ...
    cards = SettingsLoader().get_japan_cards("traffic-accident")

    for card in cards:
        print(f"{card}の処理中...")
        JapanDocument(card).save()
        print(f"{card}の処理完了")
