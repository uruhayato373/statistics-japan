...
from document_processer.main import CityDocumentProcessorClass  # noqa: E402


class CityDocument:
    """
    クラス:
        CityDocument: 市区町村別データの処理と保存を行うクラス
    ...
    def __init__(self, path: str):
        ...
        self.document = {
            ...
            **CityDocumentProcessorClass(self.categories).result,
        }
        self.dir = self._create_directory()

    def _create_directory(self) -> tuple:
        dir = f"src/data/documents/{self.settings['fieldId']}/{self.settings['menuId']}/city/{self.documentId}"
...
if __name__ == "__main__":
    ...
    cards = SettingsLoader().get_city_cards("traffic-accident")

    for card in cards:
        print(f"{card}の処理中...")
        CityDocument(card).save()
        print(f"{card}の処理完了")
