...
export async function deleteColumn(columnId) {
  ...
  mutate(
    ...
    (currentBacklog) => {
      ...
      const columns = backlogs.columns.filter(
    ...
    (currentBacklog) => {
      ...
      if (storyId) {
        const currentStory = backlogs.userStory.filter(
          (story) => story.itemIds.filter((itemId) => itemId === newItem.id)[0]
        )[0]
      ...
      if (columnId) {
        const currentColumn = backlogs.columns.filter(
          (column) =>
            column.itemIds.filter((itemId) => itemId === newItem.id)[0]
        )[0]
...
export async function deleteStory(storyId) {
  ...
  mutate(
    ...
    (currentBacklog) => {
      ...
      const userStory = backlogs.userStory.filter(
        (column) => column.id !== storyId
      )
      const userStoryOrder = backlogs.userStoryOrder.filter(
        (id) => id !== storyId
      )
